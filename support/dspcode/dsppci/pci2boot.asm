...
08Mar04 set AUX1 to 1 for gen1/2 switchable boards
20Aug04 last change MPL

ARC, Inc. comments below....
...
CHANGES, February to March 2001
...
; MPL added 12/19/03
	BCLR    #INTA,X:DCTR                ; $8073 - Clear PCI interrupt
	DC	0
...
;   the timing board
	...
	MOVEP	#$000809,X:CRA0	; Divide 78.4 MHz by 20 to get 3.92 MHz
				...
				; WL0-WL2 = ALC = 0 for 8-bit data words
	...
	MOVEP	#%010101,X:PDRC	; Data Register - ROM/FIFO* = 0, SC02 = 0,
				;   AUX1 = 1, AUX2 = AUX3 = 1 (MPL for Gen2)
...
L_RESET
	...
	MOVEP	#$000000,X:TX00
	MOVEP	#%010000,X:PDRD		; Reset the receiver FIFO
	REP	#10
	NOP
	MOVEP	#%010100,X:PDRD		; Stop resetting the FIFO

; Wait around for the 'SYR' reply from the controller
	JSR	<RD_FO_TIMEOUT		; Move the FIFO reply into A1
	JCS	<ERROR
	CMP	#$020002,B
	JNE	<ERROR			; There was an error
	JCLR	#EF,X:PDRD,*		; Test for new fiber optic data
	JSR	<RD_FO_TIMEOUT		; Move the FIFO reply into A1
	JCS	<ERROR
	CMP	#'SYR',B
	JNE	<ERROR			; There was an error
...
; Compute the number of pixels to read from the controller
	...
	MOVE	B0,X:<IPXLS_0
	MOVE	B0,X1			; 1/2 the FIFO depth
	MOVE	X:<C512,X0
	MOVE	X:<PCI_ADDR_1,B1	; B = current PCI address
...
; (a) Write complete 512 pixel (1/2 FIFO) image blocks to the PCI bus
...
C_RPXLS	CLR	A
	...
	MOVE	B1,A1
	NOP
	SUB	Y,A			; Current PCI address - BASE address
...
; Test Y: memory mapped to AA0 and AA2 from $000000 to $FFFFFF (16 megapixels)
	CLR	A
	MOVE	#$10000,X0		; Y:$000000 to Y:$FEFFFF
	MOVE	A,R0
	DO	#$FF,L_WRITE_RAM1
	DO	X0,L_WRITE_RAM0	
	MOVE	A1,Y:(R0)+
...
; Test X: memory mapped to AA3 from $1000 to $7FFFFF (8 megapixels)
	...
	MOVE	#$1000,X0		; X:$001000 to X:$7FFFF
	DO	#$7FF,L_WRITE_RAM3
	DO	X0,L_WRITE_RAM4	
	MOVE	A,X:(R0)+
	ADD	#1,A
...
L_CHECK_RAM5
	...
	JMP 	<FINISH			; Successful memory test
...
TRM_BUSY
	MOVE	X:<FLAG_BUSY,Y1		; Flag = 6 => controller busy
	MOVE	Y1,X:<HOST_FLAG	
	JSR	<FO_WRITE_HOST_FLAG
	RTS
